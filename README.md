...
- [ ] **Phase 1** — Local skeleton (monorepo, Next.js, packages, tooling)
- [ ] **Phase 2** — DB & Auth (Postgres, Prisma, RLS, Better Auth, audit log)
- [ ] **Phase 3** — Subdomain routing & UI shell (middleware, route groups, shadcn, i18n)
...
> Paste this section into a new Claude Code session via `/compact` to retain cross-session context.

**Project:** SaaS platform "DutyHive" (working name, rebrand-flexible). Solo dev based in Austria. Free during Beta, commercial later (legal entity TBD). Healthcare focus, but not exclusive. Cannot ship medical devices initially.

**Three planned sub-products** (Foundation = shell only, NO product features yet):

1. Planner — `planner.dutyhive.com` — personal shift planner, PWA, ICS export.
2. Business — `business.dutyhive.com` — multi-tenant org shift management.
3. Checklist — `checklist.dutyhive.com` — configurable checklists.

Same auth and accounts across all three. Marketing site lives at the apex `dutyhive.com`. `app.dutyhive.com` is the cross-tool account hub (login, profile, org switcher).

**Stack:** Next.js 15 (App Router) · TypeScript strict · Tailwind 4 · shadcn/ui · pnpm 10 + turborepo · Better Auth (with `organization` plugin) · Prisma 6 + Postgres 17 + Row-Level Security · Resend · Trigger.dev v3 Cloud · Sentry EU · next-intl · Serwist (PWA) · Vitest + Playwright · pino · Husky + lint-staged + commitlint.

**Hosting:** Hetzner Falkenstein. Three VPSes via Coolify: `mgmt-01` CX22 (Coolify + Beszel hub), `app-01` CPX21 (Next.js), `db-01` CPX21 (Postgres, internal-only listener). Plus Hetzner Storage Box BX11 (backups) + Object Storage (assets). DNS at Cloudflare (migrating from Vercel registrar). Beszel for metrics.
...
**Subdomain routing:** Single Next.js app. Middleware reads `Host` header, calls `resolveSubdomain` (`packages/config/src/subdomains.ts`), and rewrites the URL to `/_sub/<sub>/...`. Route groups `(marketing)`, `(account)`, `(planner)`, `(business)`, `(checklist)` live under `app/_sub/[sub]/`. Cross-subdomain cookies use domain `.dutyhive.com` (or `.lvh.me` in dev).

**Languages:** UI strings in German only (initial). Code, code comments, `/docs` all English. Assistant replies to user in German. EN i18n is a stub with `__EN_TODO__` prefix on every string.
...
**Brand-name discipline:** "DutyHive" appears ONLY in `packages/config/src/brand.ts` and `packages/i18n/src/messages/<locale>/*.json`. Code identifiers (DB schemas, package names, file paths beyond brand config) are generic where possible. Repo and packages namespaced `@dutyhive/*` by explicit user choice.

**Out of Foundation scope:** GitHub Actions (manual `pnpm test:all` before tag, per user preference), billing, status page, full marketing copy, product features, Keycloak (migration path documented for future SAML/SSO needs), analytics, audit-log UI, EN translations, 2FA enrollment UI, WAL archiving / PITR.

**Legal gates:** Austrian lawyer review of `docs/legal/impressum.de.md`, `datenschutz.de.md`, `agb.de.md` is **mandatory** before charging any user (ADR-0010). DPA checklist in `docs/legal/dpa-checklist.md` lists every subprocessor and DPA status.

**Subprocessors to document/sign:** Hetzner (DE), Cloudflare (US/EU), Resend (EU), Sentry (EU), Trigger.dev (US, EU beta), GitHub (US), Vercel (US, registrar only).

**Languages convention reminder for the assistant:** reply to the principal in German. Code, comments, docs in English.

**Critical files** (load these to orient quickly):

- `apps/web/middleware.ts` — subdomain detection, request-id, RLS context bridge entry point
...
- `README.md` — this file (project intro + this `/compact` section)
- `docs/architecture/adr/` — load relevant ADR(s) when changing architectural choices

**Current phase:** `Foundation Phase 1/7 — local skeleton`. Last touched: 2026-05-04. Next item: complete Phase 1 verification (`pnpm install`, `pnpm typecheck`, `pnpm build`), then move to Phase 2 (DB & Auth).
